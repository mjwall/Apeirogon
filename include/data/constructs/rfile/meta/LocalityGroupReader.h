...
class LocalityGroupReader : public FileIterator
{
protected:
...
public:
    ...
    void
    seek (StreamRelocation *position)
    {
        StreamSeekable *newSeekRequest =
            dynamic_cast<StreamSeekable*> (position);
        if (closed)
        {
            throw IllegalArgumentException ("Locality group reader closed");
        }
        ...
        if (interrupted)
        {
            throw IterationInterruptedException ("interrupted");
        }
    ...
    getDataBlock (uint32_t index)
    {
        ...
        InputStream *stream = region->readDataStream (reader);
        delete region;
        return stream;
    }
