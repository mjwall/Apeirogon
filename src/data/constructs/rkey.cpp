...
RelativeKey::RelativeKey () :
    fieldsSame (0), fieldsPrefixed (0), key(NULL), prevKey(NULL)
{

}

int
RelativeKey::commonPrefix (std::pair<char*, size_t> prev,
                           std::pair<char*, size_t> curr)
{
    ...
    int maxChecks = min (prevLen, curLen);
    int common = 0;
...
StreamInterface *
RelativeKey::getStream ()
{
    return key;
}

void
RelativeKey::setBase (Key *my_key)
{
    if (my_key != NULL)
    {
        if (NULL != key)
...
void
RelativeKey::setPrevious (Key *previous_key)
{
    if (previous_key != NULL)
    {
        if (NULL != prevKey)
...
uint64_t
RelativeKey::read (InputStream *stream)
{
    ...
    prevCf.insert (prevCf.begin (), scratch.first,
                   scratch.first + scratch.second);
    scratch = prevKey->getColQualifier ();
    prevCq.insert (prevCq.begin (), scratch.first,
                   scratch.first + scratch.second);
    ...
    readPrefix (stream, &rowCmp, RelativeKey::ROW_SAME,
                          RelativeKey::ROW_PREFIX, fieldsSame,fieldsPrefixed, &row,
                          &prevRow);
...
void
RelativeKey::readPrefix (InputStream *stream, vector<char> *row, vector<char> *prevRow)
{
...
uint64_t
RelativeKey::write (OutputStream *outStream)
{
...
bool
RelativeKey::operator < (const RelativeKey &rhs) const
{
    return key < rhs.key;
}
...
void
RelativeKey::setKey (Key *keyToCopy, Key *keyToCopyTo)
{
...
bool
RelativeKey::isSame (std::pair<char*, size_t> a, std::pair<char*, size_t> b)
{
