...
uint64_t
HadoopDataOutputStream::getPos ()
{
    return output_stream_ref->getPos ();
}

uint64_t
HadoopDataOutputStream::write (const char *bytes, long cnt)
{
    return output_stream_ref->write (bytes, cnt);
}

uint64_t
HadoopDataOutputStream::writeByte (int byte)
{
    return output_stream_ref->writeByte (byte);
}

uint64_t
HadoopDataOutputStream::writeString (string s)
{
...
uint64_t
HadoopDataOutputStream::write (const uint8_t *bytes, long cnt)
{
    return output_stream_ref->write (bytes, cnt);
}

uint64_t
HadoopDataOutputStream::writeBytes (const uint8_t *bytes, size_t cnt)
{
    return output_stream_ref->writeBytes (bytes, cnt);
}

uint64_t
HadoopDataOutputStream::writeByte (const uint8_t byte)
{
    return output_stream_ref->writeByte (byte);
}

uint64_t
HadoopDataOutputStream::writeShort (const short shortVal)
{
    return output_stream_ref->writeShort (shortVal);
}
uint64_t
HadoopDataOutputStream::writeInt (const int intVal)
{
    return output_stream_ref->writeInt (intVal);
}

uint64_t
HadoopDataOutputStream::writeLong (const uint64_t val)
{
    return output_stream_ref->writeLong (val);
}

uint64_t
HadoopDataOutputStream::writeBoolean (const bool val)
{
...
uint64_t
HadoopDataOutputStream::writeHadoopLong (const int64_t n)
{
    if ((n < 128) && (n >= -32))
    {
